...
protected:
  ...
  float _autoMin = 10; // lowest raw value the autoLevel will fall to before stopping. -1 = will auto level down to 10.
...
void AudioAnalysis::computeFFT(int32_t *samples, int sampleSize, int sampleRate)
{
  ...
  if (_FFT == nullptr || _sampleSize != sampleSize || _sampleRate != sampleRate) 
  {
    ...
    _FFT = new EspFFT<float>(_real, _imag, _sampleSize, _sampleRate, _weighingFactors);
  }
  ...
  _FFT->complexToMagnitude(SAMPLE_RATE, SAMPLE_SIZE);                                        /* Compute magnitudes */
}
...
void AudioAnalysis::computeFrequencies(uint8_t bandSize) // converts FFT data into frequency bands
{
